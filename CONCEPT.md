...
- **Server-Variante (eingebaut, nicht separat)**: Die Desktop-App ist immer der Client. Server-Modus ist optional — in Settings → Server URL eingeben → App verbindet sich über Gateway API (REST + WebSocket) zum Server. UI bleibt identisch ob lokal oder server-verbunden. Server = ASP.NET Core Backend (headless, Raspberry Pi, Docker ARM64+x64). Zwei Zugangswege: (1) Gateway API für die Desktop-App — holt Filamente/Drucker/Einstellungen, sendet Druckjobs, live WebSocket für Temperaturen/Fortschritt. (2) Web-UI für Browser-Zugriff (Handy/Tablet ohne Desktop-App). Remote-Zugang via Tailscale/WireGuard/Reverse Proxy. Multi-PC Sync über denselben Server. Server abschaltbar → App fällt zurück auf lokales SQLite.
