...
- **Server-Variante (eingebaut, nicht separat)**: Die Desktop-App ist immer der Client. Server-Modus ist optional. **Auto-Discovery**: App und Server finden sich selbst via mDNS/Bonjour (UDP Broadcast) — keine manuelle IP-Eingabe nötig, App zeigt gefundene Server an. Manuelle Verbindung für Remote/VPN möglich. Server = ASP.NET Core Backend (headless), läuft auf **jedem Linux-Server** (Raspberry Pi, NUC, VPS, alter Laptop, NAS mit Docker — ARM64 + x64). Zwei Zugangswege: (1) Gateway API für die Desktop-App (REST + WebSocket — holt Filamente/Drucker/Einstellungen, sendet Druckjobs, live Updates). (2) Web-UI für Browser-Zugriff (Handy/Tablet ohne Desktop-App). Remote-Zugang via Tailscale/WireGuard/Reverse Proxy. Multi-PC Sync über denselben Server. Server abschaltbar → App fällt zurück auf lokales SQLite.
